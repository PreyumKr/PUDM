...
def add_poisson_noise(pts, sigma, clamp, rate=3.0):  # Fix typo: possion -> poisson
    # input: (b, 3, n)
    assert (clamp > 0)
...
if __name__ == '__main__':
    ...
    R = 4
    noise_level = 0.1
    noise_type = "poisson"  # random, gaussian, laplace, poisson
    ...
    parser.add_argument('--noise_type', default=noise_type, help='random/gaussian/laplace/poisson')  # Fix typo
    ...
    print(f"---- points : {args.input_pts_num}, R : {args.R}, noise_type : {args.noise_type}, noise level : {args.noise_level}----")
    ...
    # Remove the noise suffix from the directory name
